...
int choice(int number, int f[9][2])            //Algorytm wyszukuj¹cy odpowiedni¹ liczbe  z f(i)
{
	int column = 0;
	while (column <= 6)
	{
...
int max(int tab[45][6], int number)               //Algorytm wyszukuj¹cy najwy¿szy wzrost.
{
	int row = 0;
	int greatest = 0;
	while (row <= 45)
...
int main()
{
	...
	while (row <= 8)
	{
		...
		if (tab3[row][column] < 1)                            //if po to by siê dobrze drukowa³o 
			cout << " ";
	...
	while (row <= 8)
	{
		...
		if (tab0[row][column - 2] < 1)                            //if po to by siê dobrze drukowa³o 
			cout << " ";
